...
* **API Token** (опционально, строка вида `ctfd_...`)
  ...
  * Добавляется в заголовок:
    `Authorization: Token <твой_токен>` — так требует CTFd API. ([docs.ctfd.io][1])
...
* **Cookie строка** (опционально)
  ...
  * Нужна, если CTFd защищён site-password или нужна какая-то особая сессия. ([docs.ctfd.io][2])
...
1. **Создаётся httpx.AsyncClient** с нужными заголовками и cookies.
   ...
   * В заголовки кладётся `Authorization: Token ...` при наличии токена. ([docs.ctfd.io][1])
   * Для запросов к API принудительно ставится `Content-Type: application/json`, чтобы обойти баг, когда токены без этого заголовка могут не приниматься. ([GitHub][3])
...
3. Если введён URL списка (`.../challenges`):
   ...
   * Сначала вызывается `/api/v1/challenges` → берётся **полный список задач с ID**. ([docs.ctfd.io][4])
...
4. Для каждой задачи запускается `scrape_ctfd_challenge(...)`:
   ...
   * По ID вызывается `/api/v1/challenges/<id>` → **название, категория, стоимость, описание, файлы**. ([docs.ctfd.io][1])
